...
cbigStep (While b c, s) = 
  case bbigStep (b,s) of
    True -> let (_, s') = cbigStep (c, s) in cbigStep (While b c, s')
    False -> (Skip, s)

cbigStep (DoWhile c b,s) = 
  let (_, s') = cbigStep (c,s) in
  case bbigStep (b,s') of
    True -> cbigStep (DoWhile c b,s')
    False -> (Skip, s')
...
teste3 :: B
teste3 = (Leq (Soma (Var "x") (Num 10))  (Mult (Num 2) (Num 3)))
...
testec2 :: C
testec2 = (Seq (Seq (Atrib (Var "z") (Num 10)) (Atrib (Var "x") (Num 3))) 
               (Atrib (Var "y") (Var "z")))
